...
function isKeNumber(number){
	var valid=true;
	var first3chars = number.substring(0, 3);
	var allowed = new Array("070","071","072","073","075","077","078");
	if($.inArray(first3chars, allowed) == -1) {
		valid=false;
	}
	return valid;
}
function isValidPhone(number, country){
	if(country.toLowerCase() == 'ug'){
		return isUgNumber(number);
	}
	if(country.toLowerCase() == 'ke'){
		return isKeNumber(number);
	}
	return false
}
