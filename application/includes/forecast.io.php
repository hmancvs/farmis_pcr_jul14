...
class ForecastIO{
  ...
  private function requestData($latitude, $longitude, $timestamp = '', $exclusions = '') {
    
    $request_url = self::API_ENDPOINT.$this->api_key.'/'.$latitude.','.$longitude;
    if(!isEmptyString($timestamp)){
    	$request_url .= ','.$timestamp; 
    }
    $request_url .= '?units=auto';
  	if(!isEmptyString($exclusions)){
    	$request_url .= '&exclude='.$exclusions; 
    } 
    	
    $client = new Zend_Http_Client($request_url);
    // debugMessage($request_url);
	try {
		// debugMessage('starting to broadcast'); // exit;
		$response = $client->request();
		$content = $response->getBody();
		// debugMessage(json_decode($content));
   ...
   */
  function getCurrentConditions($latitude, $longitude, $timestamp='', $exclusions='minutely,hourly') {
   ...
   * @return \ForecastIOConditions|boolean
   */
  function getHistoricalConditions($latitude, $longitude, $timestamp, $exclusions = 'currently,minutely,hourly,alerts,flags' ) {
    $data = $this->requestData($latitude, $longitude, $timestamp, $exclusions);
    if ($data !== false) {
   ...
   * @return \ForecastIOConditions|boolean
   */
  function getForecastToday($latitude, $longitude, $timestamp='', $exclusions='minutely') {
    $data = $this->requestData($latitude, $longitude, $timestamp, $exclusions);
    if ($data !== false) {
      $conditions = array();
      $today = date('Y-m-d');
   ...
   * @return \ForecastIOConditions|boolean
   */
  function getForecastWeek($latitude, $longitude, $timestamp='', $exclusions='minutely,currently,hourly') {
    $data = $this->requestData($latitude, $longitude, $timestamp, $exclusions);
    if ($data !== false) {
      $conditions = array();
      // debugMessage($data->daily->data);
      foreach ($data->daily->data as $raw_data) {
        $conditions[] = new ForecastIOConditions($raw_data);
      }
      // debugMessage($conditions{0});
      return $conditions;
    } else {
      return false;
    }
...
class ForecastIOConditions{
  ...
  function getMinTemperature() {
	...
	function getMinTemperatureTime($format=null) {
		if (!isset($format)) {
	      return $this->raw_data->temperatureMinTime;
	    } else {
	      return date($format, $this->raw_data->temperatureMinTime);
	    }
 	}
  /**
   * get max temperature
  ...
  function getMaxTemperature() {
	...
	function getMaxTemperatureTime($format=null) {
		if (!isset($format)) {
	      return $this->raw_data->temperatureMaxTime;
	    } else {
	      return date($format, $this->raw_data->temperatureMaxTime);
	    }
 	}
  /**
