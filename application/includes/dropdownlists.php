<?php
	...
	*/
	...
	function getSubscriptionPeriods(){
		return array('365'=>'1 Year', '182'=>'6 Months', '91'=>'3 Months', '30'=>'1 Month');
	}
	...
	function getProfilingUsers($country = 'UG'){
		$query = "SELECT u.id as optionvalue, concat(u.firstname, ' ', u.lastname) as optiontext FROM useraccount AS u WHERE (u.type <> '2') AND u.country = UPPER('".$country."') order by optiontext asc ";
		return getOptionValuesFromDatabaseQuery($query);
	}
	...
	function getServiceStatus($value = ''){
		$array = array(0 =>'Inactive', 1 => 'Active', 2=>'Expired');
		if(!isEmptyString($value)){
			return $array[$value];
		}
		return $array;
