...
class FarmerController extends ProfileController   {
	...
	public function getActionforACL() {
		...
		if($action == "add" && !isEmptyString($this->_getParam('id')) || $action == "changepassword" || $action == "processchangepassword" || $action == "changeusername" || 
		...
		if($action == "username" || $action == "gps" || $action == "search" ||
			...
			$action == "privacy" || $action == "resetprivacy" || $action == "processadd" ||
	...
	public function listAction(){
		...
		if(isFarmGroupAdmin() && isEmptyString($this->_getParam('farmgroupid'))){
			...
			$this->_helper->redirector->gotoUrl($this->view->baseUrl("farmer/list/farmgroupid/".$user->getFarmGroupID()));
		}
		// $this->listAction();
	}
	...
	public function processpictureAction() {
		...
		if($upload->receive()){
			...
			try {
				...
				$this->_helper->redirector->gotoUrl($this->view->baseUrl("farmer/picture/id/".encode($user->getID()).'/crop/1/type/'.$type));
			} catch (Exception $e) {
				$session->setVar(ERROR_MESSAGE, $e->getMessage());
				$session->setVar(FORM_VALUES, $this->_getAllParams());
				$this->_helper->redirector->gotoUrl($this->view->baseUrl('farmer/picture/id/'.encode($user->getID()).'/type/'.$type));
			}
		} else {
	...
	function changeemailAction()  {
		...
		if(!isArrayKeyAnEmptyString('actkey', $formvalues) && !isArrayKeyAnEmptyString('ref', $formvalues)){
			...
			if($formvalues['actkey'] != $user->getActivationKey()){
				...
				$failureurl = $this->view->baseUrl('farmer/view/id/'.encode($user->getID()).'/tab/account');
			...
			$this->_helper->redirector->gotoUrl($this->view->baseUrl('farmer/view/id/'.encode($user->getID()).'/tab/account'));
		}
	}
	...
	function resendemailcodeAction()  {
		...
		$session->setVar(SUCCESS_MESSAGE, $successmessage);
		$this->_redirect($this->view->baseUrl('farmer/view/id/'.encode($user->getID()).'/tab/account'));
	}
